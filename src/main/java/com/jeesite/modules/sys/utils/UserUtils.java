...
public class UserUtils {
	...
	public static List<Menu> getMenuList() {
		...
		if (menuList == null) {
			...
			// 在这个位置重构menuList
			menuList = reBindMenuList(menuList);
			putCache(CACHE_MENU_LIST, menuList);
		}
		return menuList;
	}

	public static List<Menu> reBindMenuList(List<Menu> listMenu) {
		...
		for (int i = 0; i < listMenu.size(); i++) {
			int tempLength = listMenu.get(i).getParentIds().split(",").length;
			switch (tempLength) {
			case 0:
				listMenu.get(i).setDeep(0);
				break;
			case 1:
				listMenu.get(i).setDeep(1);
				break;
			case 2:
				listMenu.get(i).setDeep(2);
				break;
			case 3:
				listMenu.get(i).setDeep(3);
				break;
			case 4:
				listMenu.get(i).setDeep(4);
				break;
			}

			int childCount = 0;
			for (Menu m1 : listMenu) {
				if (m1.getParent().getId().equals(listMenu.get(i).getId())) {
					childCount = childCount + 1;
				}
			}
			listMenu.get(i).setChildCount(childCount);
